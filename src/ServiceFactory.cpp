...
Building *ServiceFactory::createBuilding(BuildingType type, Citizen& owner) {
    ...
    if(type == BuildingType::PoliceStation) {
        ...
        Building* b = new PoliceService(300, location,resources, 300, owner, id, BuildingType::PoliceStation);
        taxAuthority->registerBuilding(*b);
        return b; 
    } else if(type == BuildingType::Hospital) {
        int id = numberOfHospitals;
        numberOfHospitals++;
        Building* b = new HealthService(500, location,resources, 500, owner,BuildingType::Hospital,id);
        taxAuthority->registerBuilding(*b);
        return b; 
        ...
        numberOfSchools++;
        Building* b = new EducationService(200, location,resources, 1000, owner, id,BuildingType::School);
        taxAuthority->registerBuilding(*b);
        return b; 
    }
}
